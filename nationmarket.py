        ...
        if cells[0].get_text() == "Ships":
            ...
            if unitQuantity > 0:
                ...
                if moneyNeeded < bankAmount:
					...
					print ('Purchased ' + locale.format("%d", unitQuantity, grouping=True) + ' ' + unitType + ' for your Nation Army at ' + locale.currency(unitPrice, grouping=True) + ' costing '  + locale.currency(int(moneyNeeded), grouping=True) + '. You have ' + locale.currency(int(moneyLeft), grouping=True) + ' left in your Nation bank.')
